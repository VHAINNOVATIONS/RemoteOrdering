...
uses fOCAccept, uODBase, rCore, fODMessage,
  {uEventHooks,} VAUtils, uBcmaOrder, System.Types;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        List := TStringList.Create;
        Inc(i);
        if Pos('priorit',LowerCase(Name))>0 then
        begin
          while (i < Src.Count) and (CharAt(Src[i], 1) <> '~') do
          begin
            DfltUrgency := Piece(Copy(Src[i], 2, 255),'^',2);
            if (DfltUrgency = 'DONE') or (DfltUrgency = 'ZD') then
            begin
              Text := 'ZD';
              ListID := '99';
              List.Add(Text);
...
begin
  ...
  begin
    ...
    SignInfo := Patient.DFN + ';' + IntToStr(Encounter.Provider) + ';' + IntToStr(Encounter.Location);
    BcmaOrderChanges.Add(CH_ORD, NewOrder.ID, NewOrder.Text, Responses.FViewName, CanSign,
    '', 0 , '', false, false, false, SignInfo);
    BcmaOrderID := NewOrder.ID;
  end
