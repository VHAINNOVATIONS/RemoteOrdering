...
function ExecuteSessionOrderChecks(OrderList: TStringList; var OrdersDeleted: String) : Boolean;

implementation

{$R *.DFM}

uses rOrders, uCore, uBcmaOrder;
...
begin
  ...
  try
    ...
    begin
	  ...
	  begin
      if ExecuteSessionOrderChecks(OrderIDList, DeletedOrders) then
	    begin
...
{          if Assigned(frmFrame) then
    ...
    OrdersDeleted := BCMADeletedOrders;
  finally
    CheckList.Free;
  end;
end;
      ...
      begin
        ...
        if (already>=0) or (DeleteCheckedOrder(AnOrderID)) then
          begin
             ...
             if BCMADeletedOrders = '' then
               BCMADeletedOrders := AnOrderID
             else
               BCMADeletedOrders := BCMADeletedOrders + '^' + AnOrderID;
