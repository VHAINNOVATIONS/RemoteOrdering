...
type
  ...
  private
    ...
    constructor Create(AnItemType: Integer; const AnID, AText, AGroupName: string;
      ...
      DCOrder: Boolean = False; Delay: Boolean = False; ASignInfo: String = '');
  public
    ...
    property SignInfo: string read FSignInfo write FSignInfo;
  ...
  public
    ...
    procedure Add(ItemType: Integer; const AnID, ItemText, GroupName: string; SignState: Integer; AParentID: string = '';
                  User: int64 = 0; OrderDG: String = ''; DCOrder: Boolean = False; Delay: Boolean = False; ProblemAdded: Boolean = False; SignInfo: string = '');
    procedure Clear;
...
var
  ...
  //MOB
  BcmaSolutions: TStringList;
  BcmaAdditives: TStringList;
  BcmaDrugIen:   int64;
  BcmaOrderID:   String;
  BcmaSignedOrder: string;
  BcmaCanceledOrder: string;
  BcmaSignInfo:  string;
  BcmaOrderChanges: TChanges;
  BcmaIVType: Integer;
  MOBCallingAPP: String;
  MOBInstructor: String;
  JobNumber: String;
...
constructor TChangeItem.Create(AnItemType: Integer; const AnID, AText, AGroupName: string;
  ASignState: Integer; AParentID: string; user: int64; OrderDG: string; DCOrder, Delay: Boolean; ASignInfo: String);
begin
...
procedure TChanges.Add(ItemType: Integer; const AnID, ItemText, GroupName: string;
  ...
  DCOrder, Delay, ProblemAdded: Boolean; SignInfo: String);
var
  i: Integer;
  Found: Boolean;
  ChangeList: TList;
...
begin
  ...
  if not Found then
  begin
    NewChangeItem := TChangeItem.Create(ItemType, AnID, ItemText, GroupName, SignState, AParentID, User, OrderDG, DCOrder, Delay, SignInfo);
    case ItemType of
    CH_DOC: begin
...
begin
  ...
  try
    ...
    begin
      ...
      ASignInfo := Patient.DFN + '^' + IntToStr(Encounter.Provider) + '^'
      ...
      Add(CH_ORD, AnID, TextForOrder(AnID), 'Other Unsigned', CanSign,'', OrderUser, Display, IsDiscontinue, IsDelay, false, ASignInfo);
    end;
  finally
    StatusText('');
    HaveOrders.Free;
