...
begin
  ...
  try
    ...
    if Assigned(uEncPCEData) and not uEncPCEData.Inpatient then
       begin
         if uEncPCEData.UseEncounter then   //if create/edit new encounter and ambulatory, use encounter date/time
           begin
             if (ADate = 0) and not ((Encounter.VisitCategory = 'E') or (Encounter.VisitCategory = 'H')
             or (Encounter.VisitCategory = 'D')) and not Encounter.NeedVisit then ADate := Encounter.DateTime
           end
         else      //if edit note encounter and ambulatory, use note encounter date/time
           begin
             if (uEncPCEData.VisitDateTime <> 0) and not ((uEncPCEData.VisitCategory = 'E') or (uEncPCEData.VisitCategory = 'H')
             or (uEncPCEData.VisitCategory = 'D')) then ADate := uEncPCEData.VisitDateTime;
           end;
       end
    else
       if (ADate = 0) and not ((Encounter.VisitCategory = 'E') or (Encounter.VisitCategory = 'H')
...
begin
  if Assigned(uEncPCEData) and not uEncPCEData.Inpatient and not uEncPCEData.UseEncounter then
