...
//  TPCEData = class;
  ...
  private
    ...
    function GetICD10Date: TFMDateTime;
  public
    constructor Create;
    ...
    property ICD10ImplDate: TFMDateTime read GetICD10Date;
  end;
...
function TPCEData.GetICD10Date: TFMDateTime;
begin
  Result := GetICD10ImplementationDate;
end;
...
var
  cd, ICD10Date: TFMDateTime;  //compare date
begin
  // if no Enc Dt or Historical, Hospitalization, or Daily Visit compare I-10 Impl dt with TODAY
  if (FEncDateTime <= 0) or (FEncSvcCat = 'E') or (FEncSvcCat = 'H') then
    cd := FMNow
  else // otherwise compare I-10 Impl dt with note Encounter date/time
    cd := FEncDateTime;

   ICD10Date :=  ICD10ImplDate; //Only need to make one call
   if (ICD10Date > cd) or (ICD10Date = 0) then
    Result := 'ICD^ICD-9-CM'
   else
