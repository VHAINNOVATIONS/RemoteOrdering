...
uses
  AnsiStrings, SysUtils, winsock, xwbut1, WinProcs, Wintypes, Classes, Dialogs, 
...
begin
  ...
  HostName := AnsiStrings.StrNew(PAnsiChar(AnsiString(s)));
  test := inet_addr(HostName);
  if test > INADDR_ANY then begin
    Result := s;
    AnsiStrings.StrDispose(Hostname);
    if ChangeCursor then
...
begin
  ...
  if xFlush = True then begin
    ...
    AnsiStrings.StrDispose(BufRecv);
    xFlush := False;
    //Buf := nil;    //P14
...
//      BufSend := StrNew(PAnsiChar({Prefix +} imsg))  //;     //moved in P14
      BufSend := AnsiStrings.StrNew(PAnsiChar(AnsiString(imsg)));
    try  // BufSend
      Result := PChar('');
...
//                sBuf := ConCat(sBuf, BufPtr);
          ...
          sBuf := Copy(sBuf, 1, BytesTotal - 1);
  {
      ...
      AnsiStrings.StrDispose(BufSend);
    end;
...
begin
  ...
  if buflen = SOCKET_ERROR then
    NetError( 'recv',0);
  buflen := ord(s[0]);
  sb := AnsiStrings.AnsiStrAlloc(buflen+1);
  sb[0] := #0;
  ...
  sb[buflen] := #0;
  Result := string(sb);
  AnsiStrings.StrDispose(sb);
  AnsiStrings.StrDispose(s);
end;
