...
var
  ...
  i, ODItem, tempOI, ALTOI, rLevel: integer;
...
begin
  ...
  if (ResolvedDialog.DialogIEN = ClinMedsDlgIen) then
  begin
    ClinicLocationMsg := ClinicLocationMsg + 'Medication';
  end else if (ResolvedDialog.DialogIEN = ClinIVDlgIen) then
  begin
  ...
  begin
    ...
    if (uOrderSet = nil) or (not(uOrderSet = nil) and not(uOrderSetClinMedMsg)) then
    begin
    uOrderSetClinMedMsg := True;
    if (InfoBox(ClinicLocationMsg, 'Clinic Location', MB_YESNO or MB_DEFBUTTON2 or MB_ICONQUESTION) = ID_NO) then Exit;
    end;
