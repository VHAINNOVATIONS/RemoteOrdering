...
type
  ...
  private
    ...
    ChangingLeft: boolean;
    ChangingSplitter: boolean;
    ResizingAll: boolean;
    SwapPCE: boolean;
    DrawerSplitPos: integer;
    ...
    CopyPasteUpdateNeeded: boolean;
    NewNoteRunning: boolean;
    procedure DrawerEvent(ADrawer: TDrawer; ADrawerState: TDrawerState);
    ...
    procedure InsertAddendum(aParentIEN: integer = -1; aParentTitle: string = '');
    ...
    function CommandHeight: integer;
  ...
  public
    ...
    procedure SaveUserSettings(SizeArray: SettingSizeArray);
    procedure LoadUserSettings(SizeArray: SettingSizeArray);
...
begin
  ...
  Drawers.OnDrawerStateChange := DrawerEvent;
...
procedure TfrmNotes.ResizeAll;
const
  LEFT_MARGIN = 4;
var
  BottomButton: TORAlignButton;
  TopButton: TORAlignButton;
  LowerPosition: integer;
  UpperPosition: integer;
  MinPCEHeight: integer;
  TopDistance: integer;
  MinNoteWidth: Integer;
  MinSplit: integer;
begin
  if not ResizingAll then begin
    ResizingAll := True;

    // Set Visibility
    lvNotes.Visible := assigned(tvNotes.Selected) and
                       (tvNotes.Selected.ImageIndex in [IMG_TOP_LEVEL, IMG_GROUP_OPEN, IMG_GROUP_SHUT]) and
                       (not pnlWrite.Visible) and (not ReminderToggled);
    Drawers.UpdateVisual;
    splDrawers.Visible := Drawers.DrawerIsOpen;
    splList.Visible := lvNotes.Visible;
    pnlNote.Visible := (not pnlWrite.Visible) and (not ReminderToggled);
    stTitle.Visible := (lstNotes.ItemIndex <> EditingIndex) and (not ReminderToggled);
    splVert.Visible := fPCEShowing;
    memPCEShow.Visible := (not ReminderToggled) and fPCEShowing;

    if Assigned(memNewNote) then begin
      MinNoteWidth := TextWidthByFont(memNewNote.Font.Handle,
                                      StringOfChar('X', MAX_PROGRESSNOTE_WIDTH)) + (LEFT_MARGIN * 2) + ScrollBarWidth;
      if (ClientWidth - (splHorz.Left + splHorz.Width)) < MinNoteWidth  then begin
        splHorz.Left :=  ClientWidth - (MinNoteWidth - splHorz.Width) - 10;
        splMoved(splHorz);
        Self.Constraints.MinWidth := Self.Width;
      end;
    end;

    if not ChangingSplitter then begin
      ChangingSplitter := True;
      MinPCEHeight := ClientHeight - MIN_PCE_HEIGHT;
      MinSplit := abs(Font.Height) * 2;
      splHorz.Top := 0;
      splHorz.Height := Self.ClientHeight;
      if splHorz.Left < MIN_DRAWER_WIDTH then splHorz.Left := MIN_DRAWER_WIDTH;
      if splDrawers.Visible then begin
        splDrawers.Left := 0;
        splDrawers.Width := splHorz.Left;
      end;
      splList.Left := splHorz.Left + splHorz.Width;
      splList.Width := Self.ClientWidth - splList.Left;
      splVert.Left := splHorz.Left + splHorz.Width;
      splVert.Width := Self.ClientWidth - splVert.Left;
      if FormResizing then begin
        splVert.Top := ClientHeight - PCEHeight;
      end else if (splVert.Top > MinPCEHeight) then
        splVert.Top := MinPCEHeight;

      if splList.Visible and (splList.Top < MinSplit) then
        splList.Top := MinSplit;
      if splVert.Visible and ((splVert.Top + MinSplit) > ClientHeight) then
        splVert.Top := ClientHeight - MinSplit;
      ChangingSplitter := False;
    end;

    pnlReminder.Top := stTitle.Top + stTitle.Height;
    pnlReminder.Left := splHorz.Left + splHorz.Width;
    pnlReminder.Height := Self.ClientHeight - pnlReminder.Top;
    pnlReminder.Width := Self.ClientWidth - pnlReminder.Left;

    if SwapPCE then begin
      TopButton := cmdPCE;
      BottomButton := cmdNewNote;
    end else begin
      TopButton := cmdNewNote;
      BottomButton := cmdPCE;
    end;
    BottomButton.Left := 0;
    BottomButton.Width := splHorz.Left;
    TopButton.Left := 0;
    TopButton.Width := splHorz.Left;
    TopButton.TabOrder := 7;
    LowerPosition := Self.ClientHeight - FormSpace;
    if BottomButton.Visible then begin
      BottomButton.Top := LowerPosition - BottomButton.Height;
      LowerPosition := BottomButton.Top - FormSpace;
    end;
    if TopButton.Visible then begin
      TopButton.Top := LowerPosition - TopButton.Height;
      LowerPosition := TopButton.Top - FormSpace;
    end;

    if (not Drawers.DrawerIsOpen) then begin
      pnlDrawers.Height := Drawers.TotalButtonHeight;
      pnlDrawers.Top := LowerPosition - Drawers.TotalButtonHeight;
    end else begin
      pnlDrawers.Top := LowerPosition - ExpandedDrawerHeight;
      pnlDrawers.Height := ExpandedDrawerHeight;
      splDrawers.Top := pnlDrawers.Top - splDrawers.Height;
    end;

    if splDrawers.Visible then begin
      pnlDrawers.Top := splDrawers.Top + splDrawers.Height;
      pnlDrawers.Height := LowerPosition - pnlDrawers.Top;
    end else begin
      pnlDrawers.Top := LowerPosition - Drawers.TotalButtonHeight;
      pnlDrawers.Height := Drawers.TotalButtonHeight;
    end;
    pnlDrawers.Left := 0;
    pnlDrawers.Width := splHorz.Left;

    lstNotes.Top := stNotes.Height + FormSpace;
    lstNotes.Left := 0;
    lstNotes.Width := splHorz.Left;

    tvNotes.Top := stNotes.Height + FormSpace;
    tvNotes.Width := splHorz.Left;
    tvNotes.Height := pnlDrawers.Top - tvNotes.Top - splDrawers.Height;

    stTitle.Top := 0;
    stTitle.Left := splHorz.Left + splHorz.Width;

    if splVert.Visible then begin
      LowerPosition := splVert.Top;
    end else begin
      LowerPosition := Self.ClientHeight;
    end;

    if splList.Visible then begin
      UpperPosition := splList.Top + splList.Height;
    end else if lstNotes.ItemIndex = EditingIndex then begin
      UpperPosition := 0;
    end else begin
      UpperPosition := stTitle.Top + stTitle.Height;
    end;
    pnlWrite.TabStop := pnlWrite.Visible;
    pnlNote.TabStop := pnlNote.Visible;
    if pnlWrite.Visible then begin
      pnlWrite.Left := stTitle.Left;
      pnlWrite.Top := UpperPosition;
      pnlWrite.Width := Self.ClientWidth - stTitle.Left;
      pnlWrite.Height := LowerPosition - UpperPosition;
    end else begin
      if lvNotes.Visible then begin
        lvNotes.Top := stTitle.Top + stTitle.Height + FormSpace;
        lvNotes.Left := stTitle.Left;
        lvNotes.Width := Self.ClientWidth - lvNotes.Left;
        lvNotes.Height := splList.Top - lvNotes.Top; {+ splList.Height - FormSpace};
      end;
      pnlNote.Left := stTitle.Left;
      pnlNote.Top := UpperPosition;
      pnlNote.Width := Self.ClientWidth - pnlNote.Left;
      pnlNote.Height := LowerPosition - UpperPosition;
    end;
    if splVert.Visible then begin
      memPCEShow.Left := stTitle.Left;
      memPCEShow.Top := splVert.Top + splVert.Height;
      memPCEShow.Width := Self.ClientWidth - memPCEShow.Left;
      memPCEShow.Height := Self.ClientHeight - memPCEShow.Top;
    end;
    ...
    if pnlWrite.Visible then begin
      if FontChanging then begin
        cmdChange.Width := TextWidthByFont(cmdChange.Font.Handle, cmdChange.Caption) + 4;
        FontChanging := False;
      end;
    ...
    PCEHeight := ClientHeight - splVert.Top;
    TopDistance := stTitle.Height + splVert.Height + splList.Height;
    if PCEHeight > (ClientHeight - TopDistance) then begin
      PCEHeight := ClientHeight - TopDistance;
    end;
    ResizingAll := False;
  end;
end;
...
function TfrmNotes.CommandHeight: integer;
begin
  Result := FormSpace;
  if cmdPCE.Visible then inc(Result, cmdPCE.Height);
  if cmdNewNote.Visible then inc(Result, cmdNewNote.Height);
end;

procedure TfrmNotes.splMoved(Sender: TObject);
var
  NewHeight: integer;
  MinPCEHeight: integer;
begin
  if not ChangingSplitter then begin
    ChangingSplitter := True;
    if Sender = splHorz then begin
      if splHorz.Left < MIN_DRAWER_WIDTH then
        splHorz.Left := MIN_DRAWER_WIDTH
      else if splHorz.Left > (Self.ClientWidth - splHorz.Width - FormSpace) then
        splHorz.Left := Self.ClientWidth - splHorz.Width - FormSpace;
      pnlDrawers.Width := splHorz.Left;
      if splList.Visible then begin
        splList.Left := splHorz.Left + splHorz.Width;
        splList.Width := Self.ClientWidth - splList.Left;
      ...
      if splVert.Visible then begin
        splVert.Left := splHorz.Left + splHorz.Width;
        splVert.Width := Self.ClientWidth - splList.Left;
      end;
    end else if (Sender = splDrawers) then begin
      NewHeight := Self.ClientHeight - splDrawers.Top + splDrawers.Height - CommandHeight;
      if (NewHeight < Drawers.TotalButtonHeight) then begin
        splDrawers.Top := Self.ClientHeight - CommandHeight - pnlDrawers.Height - splDrawers.Height; // adjust to minimum
      end;

      if splDrawers.Top < 0 then
        splDrawers.Top := 0;
      if Drawers.DrawerIsOpen then begin
        DrawerSplitPos := splDrawers.Top;
        ExpandedDrawerHeight := Self.ClientHeight - splDrawers.Top - CommandHeight;
      end;
    end else if ((Sender = splList)) then begin
      if splList.Top < (lvNotes.Top + lvNotes.Constraints.MinHeight)  then
        splList.Top := lvNotes.Top + lvNotes.Constraints.MinHeight + FormSpace
      else if (splList.Top + splList.Height + FormSpace) > splVert.Top then
        splList.Top := splVert.Top - splList.Height - FormSpace;
    end else if ((Sender = splVert) and splVert.Visible) then begin
      MinPCEHeight := ClientHeight - MIN_PCE_HEIGHT;
      if (splVert.Top > MinPCEHeight) then splVert.Top := MinPCEHeight
      else if splVert.Top < splList.Top + splList.Height + FormSpace then
        splVert.Top := splList.Top + splList.Top + FormSpace;
    end;
    ResizeAll;
    ...
    if Sender = splHorz then begin
       if splList.Visible then begin
        splList.Left := splHorz.Left + splHorz.Width;
        splList.Width := ClientWidth - splList.Left;
      end;
      if splVert.Visible then begin
        splVert.Left := splHorz.Left + splHorz.Width;
        splVert.Width := ClientWidth - splVert.Left;
      end;
    end;
...
procedure TfrmNotes.LimitSplitters;
var
  NewHeight: integer;
  MinPCEHeight: integer;
begin
  if not ChangingSplitter then begin
    ...
    if splHorz.Visible then begin
      if splHorz.Left < MIN_DRAWER_WIDTH then begin
        splHorz.Left := MIN_DRAWER_WIDTH;
      end else if splHorz.Left > (Self.ClientWidth - splHorz.Width - FormSpace) then begin
        splHorz.Left := Self.ClientWidth - splHorz.Width - FormSpace;
      end;
      pnlDrawers.Width := splHorz.Left;
    end;
    ...
    if splList.Visible then begin
      if splList.Top < (lvNotes.Top + lvNotes.Constraints.MinHeight)  then begin
        splList.Top := lvNotes.Top + lvNotes.Constraints.MinHeight + FormSpace;
      end else if (splList.Top + splList.Height + FormSpace) > splVert.Top then begin
        splList.Top := splVert.Top - splList.Height - FormSpace;
      end;
      splList.Left := splHorz.Left + splHorz.Width;
      splList.Width := ClientWidth - splList.Left;
...
procedure TfrmNotes.InsertNewNote(IsIDChild: Boolean; AnIDParent: Integer; aNoteIEN: integer = -1; aNoteTitle: string = '');
...
begin
  ...
  try
    ...
    If LacksRequiredForCreate or (VerifyNoteTitle and (aNoteIEN < 0)) or uUnresolvedConsults.UnresolvedConsultsExist then
...
procedure TfrmNotes.InsertAddendum(aParentIEN: integer = -1; aParentTitle: string = '');
...
//procedure TfrmNotes.LoadUserSettings(posD, posH, posL, posV: integer);
procedure TfrmNotes.LoadUserSettings(SizeArray: SettingSizeArray);
var
  l: integer;
begin
  ChangingSplitter := True;
  try
    l := Length(SizeArray);
    if (l > 0) and (SizeArray[0] <> 0) then splDrawers.Top := SizeArray[0];
    if (l > 1) and (SizeArray[1] <> 0) then splHorz.Left := SizeArray[1];
    if (l > 2) and (SizeArray[2] <> 0) then splList.Top := SizeArray[2];
    if (l > 3) and (SizeArray[3] <> 0) then splVert.Top := SizeArray[3];
    PCEHeight := ClientHeight - splVert.Top;
  finally
    ChangingSplitter := False;
    ResizeAll;
...
begin
  ...
  try
    ...
    tCallV(aSmartParams, 'ORB3UTL GET NOTIFICATION', [Piece(Notifications.RecordID, '^', 2)]);
    If (aSmartParams.Values['PROCESS AS SMART NOTIFICATION'] = '1') then
      begin
        // STOLEN FROM: acNewNote.Execute;
        If Encounter.NeedVisit then
          begin
            UpdateVisit(Font.Size, DfltTIULocation);
            frmFrame.DisplayEncounterText;
        ...
        case TfrmNotificationProcessor.Execute(aSmartParams) of
          naCancel:
            ShowMessage('Cancel it all!');
          naNewNote:
            InsertNewNote(False, 0, StrToInt(aSmartParams.Values['NOTE TITLE IEN']), aSmartParams.Values['NOTE TITLE']); // aSmartNoteIEN, aSmartNoteTitle);
          naAddendum:
            InsertAddendum(StrToIntDef(aSmartParams.Values['ADDEND NOTE IEN'], -1), aSmartParams.Values['ADDEND NOTE TITLE']);
        end;
...
procedure TfrmNotes.SaveUserSettings(SizeArray: SettingSizeArray);
begin
  SetLength(SizeArray, 4);
  SizeArray[0] := splDrawers.Top;
  SizeArray[1] := splHorz.Left;
  SizeArray[2] := splList.Top;
  SizeArray[3] := splVert.Top;
end;
...
begin
  if (Value <> fExpandedDrawerHeight) then begin
    ...
    if not ChangingSplitter then ResizeAll;
  end;
end;
...
begin
  ...
  if Assigned(Drawers.RemNotifyList) then
    Drawers.RemNotifyList.free;
  KillDocTreeObjects(tvNotes);
  inherited;
end;
...
begin
  ...
  if Editing or AnytimeEncounters then begin
    ...
    if acNewNote.Visible then begin
      SwapPCE := True;
    end;
  end else begin
    acPCE.Enabled := FALSE;
...
procedure TfrmNotes.DrawerEvent(ADrawer: TDrawer; ADrawerState: TDrawerState);
begin
  ResizeAll;
end;
