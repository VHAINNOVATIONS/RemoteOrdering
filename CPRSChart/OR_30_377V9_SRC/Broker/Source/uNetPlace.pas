...
uses
  Math, Windows, Registry, StrUtils, Classes, SysUtils, uTokenTools;
...
begin
  ...
  //    Registry.RootKey := HKEY_LOCAL_MACHINE;
      ...
      if (TOSVersion.Architecture = arIntelX64) then begin
        ...
        if not Registry.KeyExists(Base) then begin
          if Registry.KeyExists('Software' + ARegKey) then begin
      ...
      begin
        ...
        for i := 0 to (WorkList.Count - 1) do begin
          cp := pos(',', WorkList[i]);
          if (Registry.GetDataType(WorkList[i]) = rdString) then begin
            DNS := Registry.ReadString(WorkList[i]);
          end else begin
            DNS := '';
          end;
          Add(TNetPlace.Create(copy(WorkList[i], 1, cp - 1), DNS, '',
                               StrToIntDef(copy(WorkList[i], cp + 1, length(WorkList[i])), 0)));
        end;
      end;
    finally
      WorkList.Free;
