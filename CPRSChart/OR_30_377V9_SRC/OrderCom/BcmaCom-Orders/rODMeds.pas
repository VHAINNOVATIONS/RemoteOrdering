...
procedure GetUDOIInfo(var AOIInfo: string; AOId: int64);
...
begin
  ...
  Result := sCallV('ORBCMA1 FAILDEA', [AnOI, Encounter.Provider, PtType]) = '1';
end;
...
begin
  if ForInpatient then PtType := 'I' else PtType := 'O';
  CallV('ORBCMA1 FORMALT', [AnIEN, PtType]);
...
begin
  if ForInpatient then PtType := 'I' else PtType := 'O';
  CallV('ORBCMA1 DOSEALT', [DispDrug, OI, PtType]);
  FastAssign(RPCBrokerV.Results, AList);
end;
...
begin
  CallV('ORBCMA1 ODSLCT', [PST_UNIT_DOSE, Patient.DFN, Encounter.Location]);
...
procedure GetUDOIInfo(var AOIInfo: string; AOId: int64);
begin
  AOIInfo := sCallV('ORBCMA5 GETUD',[AOId]);
end;
