...
function SubSetOfBCMAOrderItems(const AOIList: TStrings; const XRef: string): TStrings;
...
//MOB - Modified Code
function AmountsForIVFluidNF(AnIEN: Integer; FluidType: Char; AFlag: String): string;
...
uses TRPCB, uODBase, fODBase, uOrders;
...
//    end;
//    if ConstructOrder.DelayEvent in ['A','D','T'] then
//    begin
//      Param[7].Mult['"OREVENT"'] := ConstructOrder.DelayEvent;
//      Param[7].Mult['"OREVENT","TS"'] := IntToStr(ConstructOrder.Specialty);
//      Param[7].Mult['"OREVENT","EFFECTIVE"'] := FloatToStr(ConstructOrder.Effective);
//    end;
...
procedure CheckAuthForMeds(var x: string);
begin
  x := Piece(sCallV('ORBCMA32 AUTH', [Encounter.Provider]), U, 2);
end;
...
begin
  ...
  begin
    x := sCallV('ORBCMA32 DRUGMSG', [AnIEN]);
...
procedure LoadFormularyAlt(AList: TStringList; AnIEN: Integer; PSType: Char);
begin
  CallV('ORBCMA32 FORMALT', [AnIEN, PSType]);
...
var
  x: string;
begin
  x := sCallV('ORBCMA32 VALROUTE', [AName]);
  ID := Piece(x, U, 1);
...
function MedIsSupply(AnIEN: Integer): Boolean;
begin
  Result := sCallV('ORBCMA32 ISSPLY', [AnIEN]) = '1';
end;
...
begin
  //MOB - Modified Code
  //CallV('ORBCMA32 DLGSLCT', [PST_IV_FLUIDS, patient.dfn, patient.location]);
  CallV('ORBCMA32 DLGSLCT', [PST_IV_FLUIDS]);
  Result := RPCBrokerV.Results;
end;

function AmountsForIVFluid(AnIEN: Integer; FluidType: Char): string;
begin
  Result := sCallV('ORBCMA32 IVAMT', [AnIEN, FluidType]);
end;

//MOB - Modified Code
function AmountsForIVFluidNF(AnIEN: Integer; FluidType: Char; AFlag: String): string;
begin
  Result := sCallV('ORBCMA32 IVAMT', [IntToStr(AnIEN) + '^' + AFlag, FluidType]);
end;

function ODForMedOut: TStrings;
{ Returns init values for outpatient meds dialog.  The results must be used immediately. }
begin
  CallV('ORBCMA32 DLGSLCT', [PST_OUTPATIENT, patient.dfn, patient.location]);
  Result := RPCBrokerV.Results;
end;
...
begin
  Result := StrToIntDef(sCallV('ORBCMA32 VALSCH', [x, PSType]), -1);
end;
...
begin
  ...
  begin
    ClearParameters := True;
    RemoteProcedure := 'ORBCMA5 ODITMBC';
    Param[0].PType := literal;
    Param[0].Value := XRef;
    Param[1].PType := list;
    for i := 0 to AOIList.Count-1 do
      RPCBrokerV.Param[1].Mult[IntToStr(i+1)] := Piece(AOIList[i],';',1);
    CallBroker;
  end;
  Result := RPCBrokerV.Results
end;
