...
uses
  ...
  Menus, XUDIGSIGSC_TLB, VA508AccessibilityManager, VAUtils, Contnrs, System.UITypes, System.Types;
...
begin
  ...
  //CQ 21830 - Ignore defined quick orders so we go straight into the proper order dialogue - jcs
...
var
  AnInstr, OrderID, Text, tempOrder, tempSchString, tempSchType, AdminTime, x: string;
  i: integer;
  LocChange: boolean;
  AResponse: TResponse;
...
//      //FQOInitial := False;
//    end
    if (FActiveMedList = lstAll) and ((FOIName <> '') or (lstAll.Selected <> nil)) then  // orderable item
    begin
      if (FOiIen>0) and (FOIName <> '') then
      begin
        MedIEN := FOiIen;
        MedName := FOIName;
        txtMed.Tag := MedIEN;
...
//    FLD_PRIOR_NM  : Result := cboPriority.Text;
    FLD_PRIOR_ID  : Result := '99';
    FLD_PRIOR_NM  : Result := 'ZD';
...
begin
  ...
  //jcs if this is a supply dialogue, leave the displaygroup alone!
...
begin
  ...
  begin
    ShowMessage('Can''t find orderable Item.');
  end;
    ...
    if (Length(ScheduleCombo.Text) > 0) and (ScheduleCombo.ItemIndex < 0) and FInptDlg then
        begin
          ...
          Application.MessageBox('Please select a valid schedule from the list.'+#13+#13+
                           'Only one-time schedules are valid for one-step clinic'+
                           ' admin orders.',
                           'Incorrect Schedule.');
...
procedure TfrmODMeds.SetOI(AOiIen: int64);
var
  AOiInfo: string;
begin
  GetUDOIInfo(AOiInfo,AOiIen);
  FOiName := Piece(AOiInfo,'^',2);
  FOiIen  := AOiIen;
end;
