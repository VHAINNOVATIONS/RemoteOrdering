...
type
  ...
  protected
    ...
    procedure findLinkItem(elementList, promptList: TStringList);
  public
  ...
  TRDElemType = (etCheckBox, etTaxonomy, etDisplayOnly, etChecked);
  ...
  TRemPromptType = (ptComment, ptVisitLocation, ptVisitDate, ptQuantity,
    ...
    ptLevelUnderstanding, ptWHPapResult, ptWHNotPurp, ptDate);
...
begin
  ...
  repeat
    i := pos(CRCode, Result);
    if (i > 0) then
      Result := copy(Result, 1, i - 1) + CRLF +
        copy(Result, i + CRCodeLen, MaxInt);
  until (i = 0);
end;
...
begin
  ...
  if (IEN > 0) then
    ReportBox(DetailReminder(IEN), RemMenuNames[rmClinMaint] + ': ' +
      ReminderName(IEN), TRUE);
end;
...
begin
  IEN := (Sender as TMenuItem).Tag;
  if (IEN > 0) then
    ReportBox(EducationTopicDetail(IEN), 'Education Topic: ' +
      (Sender as TMenuItem).Caption, TRUE);
end;
...
begin
  IEN := (Sender as TMenuItem).Tag;
  if (IEN > 0) then
    ReportBox(ReminderInquiry(IEN), 'Reminder Inquiry: ' +
      ReminderName(IEN), TRUE);
end;
...
begin
  ...
  begin
    ...
    Prefix := StringOfChar(' ', 80 - LineLength);
    repeat
...
procedure TReminderDialog.findLinkItem(elementList, promptList: TStringList);
var
  a, r, p: integer;
  action: string;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        then
          continue;
        if Piece(action, U, 2) = 'CHECKED' then
        begin
        ...
        else if Piece(action, U, 2) = 'UNCHECKED' then
        begin
          Application.ProcessMessages;
          Element.SetChecked(FALSE);
          elementChange := TRUE;
        end
        else if Piece(action, U, 2) = 'UNSUPPRESS' then
        begin
          Application.ProcessMessages;
...
begin
  ...
  begin
    ...
    TempSL := GetDialogPrompts(FID, Historical, FindingType, remIEN);
...
var
  ...
  Prompt: TRemPrompt;
  RData: TRemData;
  linkItem, linkAction, linkType: String;
        ...
        if (linkItem <> '') and (linkAction <> '') and (linkType <> '') then
          begin
            if linkType = 'ELEMENT' then
              begin
                if value then elementList.Add(linkItem + U + LINKACTION)
                else elementList.Add(linkItem + U + 'UN'+linkAction);
              end
            ...
            self.FReminder.findLinkItem(elementList, promptList);
          end;
      end;
    finally
...
//                  ctrl.date.FMDateTime := StrToFloat(Prompt.Value);
                  if tmp = 'E' then
                    ...
                    ctrl.date.FMDateTime := StrToFloat(tmp);
//                  ctrl.date.FMDateTime :=  prompt.FParent.FReminder.FPCEDataObj.DateTime;
                ...
                ctrl.date.OnChange := Prompt.PromptChange;
                UpdateColorsFor508Compliance(ctrl.dt);
                DoLbl := TRUE;
                MinX := ctrl.dt.Width;
              end;
      ...
      begin
        ...
        if not FReminder.FNoResolve then
          ...
          if FReminder = CurrentReminderInDialog then
            txt := ResolveTemplateFields(CRLFText(txt), false)
          else
...
//        (Value = '0'))) or ((pt in [ptVisitDate, ptDate]) and
//        Prompt.FMonthReq and (StrToIntDef(copy(Value, 4, 2), 0) = 0)) or
//        ((pt in [ptVisitDate, ptVisitLocation, ptDate]) and (Value = '0'))))
      if (Prompt.PromptOK and (not Prompt.Forced) and Prompt.Required and
        (((pt <> ptWHNotPurp) and (pt <> ptWHPapResult)) and
        ((Value = '') or (Value = '@') or ((pt = ptPrimaryDiag) and
...
var
  ...
  begin
    PNum := -1;
    pt := Prompt.PromptType;
    if (pt = ptSubComment) or (pt = ptUnknown) then
      exit;
    if (pt = ptMST) then
    begin
    ...
    begin
      ...
      if (PNum > 0) then
      begin
        // RFR-TDrugs addition TODO: need serious review, may break PCEUpdate
  ...
  var
    i, Qty: integer;
    Value, IsGAF, txt, X, Code, Nar, Cat, GenFindID: string;
    Skip: boolean;
    Prompt: TRemPrompt;
    dt: TRemDataType;
...
begin
  ...
  if (Code = RemPromptCodes[ptVisitDate]) or (Code = RemPromptCodes[ptDate])
  then
  begin
  ...
  if (Code = RemPromptCodes[ptDate]) then
    begin
...
var
  ...
  WHValue, WHValue1, itemId, linkItem, linkType, promptValue: String;
  elementList, promptList: TStringList;
...
begin
    ...
    try
      ...
      case pt of
        ...
        ptVisitDate, ptDate:
          begin
            ...
            if pt = ptDate then
              begin
                 if self.validate = HistCode then
                  begin
                    if  dte > FMNow then
                      begin
                        InfoBox('Date cannot be after today date', 'Error', MB_OK);
                        tmpValue := OrgValue;
                        NeedRedraw := TRUE;
                      end
                  end
                  else if self.validate = FutureCode then
                    begin
                      if dte < self.FParent.FReminder.PCEDataObj.DateTime then
                        begin
                          InfoBox('Date cannot be before the encounter date', 'Error', MB_OK);
                          tmpValue := OrgValue;
                          NeedRedraw := TRUE;
                        end;
                    end;
      ...
      begin
        ...
        try
          ...
          if ((pt = ptDate) and (TmpValue = '-1')) then exit;
          if (linkItem = '') or (linkType = '') then exit;
          promptValue := getLinkPromptValue(TmpValue, itemId, linkItem,
            linkType, OrgValue, patient.dfn);
          if linkType = 'ELEMENT' then
          begin
            if promptValue <> '' then
            begin
              elementNeedRedraw := TRUE;
              elementList.Add(linkItem + U + promptValue);
            end;
          end
          else
          begin
            ...
            begin
              promptList.Add(linkItem + U + linkType + U + 'REQUIRED');
              elementNeedRedraw := TRUE;
            end
  ...
  if elementNeedRedraw then
    try
      FParent.FReminder.findLinkItem(elementList, promptList);
    finally
      if elementList <> nil then FreeAndNil(elementList);
      if promptList <> nil then  FreeAndNil(promptList);
    end;
...
begin
  ...
  if (Forced and (not FFromParent)) then
    exit;
  pt := PromptType;
  if (pt in [ptVisitDate, ptDate]) then
  begin
    if (Value = '') then
      Value := '0'
    else
...
//            infoBox('after set set', 'Warning',MB_OK);
      ...
      ptVisitDate:
        ...
        begin
          try
            (FCurrentControl as TORDateBox).FMDateTime := StrToFloat(GetValue);
          except
            on EConvertError do
...
var
  ...
  i, l: integer;
  Before, After, Score, dien, linkType, linkItem, value: string;
  MHRequired: boolean;
        ...
        begin
          ...
          for l := 0 to linkList.Count - 1 do
            begin
              ...
              if linkType = 'ELEMENT' then elementList.Add(linkItem + U + value)
              else
                begin
                  if value = 'REQUIRED' then promptList.Add(linkItem + U + linkType + U + 'REQUIRED')
                  else promptList.Add(linkItem + U + linkType + U + 'VALUE' + U + value);
                end;
            end;
            self.FParent.FReminder.findLinkItem(elementList, promptList);
      end;
    finally
//        freeAndNil(linkList);
//        if elementList <> nil then freeAndNil(elementList);
